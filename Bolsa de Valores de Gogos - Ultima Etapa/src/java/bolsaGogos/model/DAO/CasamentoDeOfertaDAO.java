...
import bolsaGogos.model.DAO.interfaces.CasamentoDeOfertaDAOInterface;
...
public class CasamentoDeOfertaDAO implements CasamentoDeOfertaDAOInterface {
    ...
    */
    ...
    public List<CasamentoDeOferta> getListaCasamento() {
        ...
        List<CasamentoDeOferta> listaCasamento = new ArrayList<>();
    ...
    */
    ...
    /**
    * Recupera os casamentos que ocorreram para uma oferta de compra
    *
    */
    @Override
    public List<CasamentoDeOferta> getCasamentosOfertaCompra(int idOferta) {
        List<CasamentoDeOferta> casamentos = new ArrayList<>();
        CasamentoDeOferta casamento = null;
        ...
        try (Connection conn = config.conectar()) {
            ...
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM casamentosoferta WHERE idOfertaCompra = ?");
            ps.setInt(1, idOferta);

            ResultSet rs = ps.executeQuery();

            while (rs.next()) {
                casamentos.add(montaCasamento(rs));
            }
    ...
    */
    ...
    public List<CasamentoDeOferta> getCasamentosOfertaVenda(int idOferta) {
        ...
        try (Connection conn = config.conectar()) {
            ...
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM casamentosoferta WHERE idOfertaVenda = ?");
            ps.setInt(1, idOferta);
